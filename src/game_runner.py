...
class GameRunner:    
    ...
    def build_game_context(
        ...
        context += "\n".join(context_lines)
        context += f"\nIn total, A chose \"Cooperate\" {game.state.player_a_cooperations} times "
        context += f"and chose \"Defect\" {len(game.state.history) - game.state.player_a_cooperations} times, "
        context += f"B chose \"Cooperate\" {game.state.player_b_cooperations} times "
        context += f"and chose \"Defect\" {len(game.state.history) - game.state.player_b_cooperations} times.\n"
    ...
    def run_tournament(
        ...
        for player_a_config, player_b_config in matchups:
            logger.info(f"\n{'='*60}")
            logger.info(f"Matchup: {player_a_config['name']} vs {player_b_config['name']}")
            logger.info(f"{'='*60}")
